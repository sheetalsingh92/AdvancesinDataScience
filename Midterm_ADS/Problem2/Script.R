...
conMatrix<-confusionMatrix(Status.test,tree.pred)
View(conMatrix)
...
Errorpercentage_classification_tree <- Error_classification_tree *100
View(Errorpercentage_classification_tree)

#create ROC curve
install.packages("ROCR")
library(ROCR)
prediction <- prediction(tree.pred, Status_test$Status)
performance <- performance(prediction, measure = "tpr", x.measure = "fpr")
plot(performance, main="ROC curve", xlab="1-Specificity", ylab="Sensitivity")
...
#create confusion matrix
conMatrix<-confusionMatrix(test$status_updated,pred)
View(conMatrix)
...
Errorpercentage_neural_network <- Error_neural_network *100
View(Errorpercentage_neural_network)
